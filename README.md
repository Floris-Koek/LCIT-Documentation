...
## Architecture: one shared `standards.md`, two skills

Both skills reference the **same** `standards.md`, which lives at the plugin level (`plugins/integration-diagram-tools/shared/standards.md`) rather than being duplicated inside each skill folder. Each `SKILL.md` points to it with a relative path (`../../shared/standards.md`). This works because the whole plugin directory — including `shared/` — is copied as one unit when a user installs the plugin, so the relative path always resolves.

This means **updating the standards once updates both skills automatically** — there's no risk of the two copies drifting apart, because there's only one copy.
...
2. Install `integration-diagram-tools` — this gives you both `mulesoft-documentation-skill` and `frends-documentation-skill`.
...
Missing functionality, or need a whole new type of diagram/documentation (e.g. a third platform)? Let Floris or Sophie know, or add a new folder under `plugins/integration-diagram-tools/skills/` yourself following the same structure (`SKILL.md`, `references/`, `assets/`), referencing `../../shared/standards.md` the same way the existing two skills do.
